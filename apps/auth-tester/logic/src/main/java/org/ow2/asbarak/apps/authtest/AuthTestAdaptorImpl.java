...
public class AuthTestAdaptorImpl implements AuthTestAdaptorService {
	...
	public String getUserInformations(Integer id, long token){
		...
		if (session.isStillValid() && session.getUserId().equals(id)) {			
		...
		// TODO we'll set the token into the security context in order to be able to identify the session
		
		
		// then we can call the logic, auth will automatically check if user associated 
